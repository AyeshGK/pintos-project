...
//define helper function for main entry
int view_on_interactive_shell(char *buff);
char* get_buffer_for_interactive_shell(char *buff);
...
pintos_init (void)
{
    ...
    if (*argv != NULL) {
        ...
        while(1){
            ...
            //get char by char form shell
            buff = get_buffer_for_interactive_shell(buff);

            if(view_on_interactive_shell(buff)==0)
            {
                break;
            }
...
//add helping fuctions for main entry point
//get buffer
char* get_buffer_for_interactive_shell(char *buff)
{
    char chr;
    int i=0;
    while (chr=input_getc()){

        //when enter is pressed then stop the buffer continuing
        if(chr == 13){
            break;
...
//view on interactiveshell
int view_on_interactive_shell(char *buff)
{
    // printf("hello brother ",buff);

    // printf("strcmp -> %d\n",);
    if(strcmp("whoami",buff)==0){
        printf("Ayesh Vininda 190649F\n");
    }else if(strcmp("shutdown",buff)==0){
        printf("shutdown...\n");
        shutdown_power_off();
        ...
        printf("%d seconds passed since 00:00:00 UTC on 1 January 1970.\n",time);
    }else if(strcmp("ram",buff)==0){
        printf ("Pintos booting with %'"PRIu32" kB RAM...\n",init_ram_pages * PGSIZE / 1024);

        // printf("offset -> %d\n",pg_ofs(1));
    }else if(strcmp("thread",buff)==0){
        thread_print_stats();
    }else if(strcmp("priority",buff)==0){
        printf("Current thread priority -> %d\n",thread_get_priority());
    }else if(strcmp("exit",buff)==0){
        printf("Exit the interactive shell\n");
        return 0;
    }else{
        printf("Check the %s command again.\n",buff);
    }
