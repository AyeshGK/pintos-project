...
void thread_unblock(struct thread *t)
{
  ...
  /*
  add thread into ready list in prioratied order
  */
...
void thread_yield(void)
{
  ...
  /*
  when yielding current thread must be re inserterd into ready list and it should be prioratied order
  */
...
void thread_set_priority(int new_priority)
{
  ...
  int priority =-1;
  if(!list_empty(&cur->doners))
  {
    struct thread* p = list_entry(list_front(&cur->doners),struct thread,d_elem);
    priority = p->priority;
...
init_thread(struct thread *t, const char *name, int priority)
{
  ...
  /*add aditional data structures to need in priority donation*/
  list_init(&t->doners);

  // list_push_back(&priority);
  /*set original priority as priority as initial*/
